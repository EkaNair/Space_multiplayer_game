...
pygame.display.set_caption("Space 1v1 game")
...
mixer.music.load('the_weird_beat.mp3')
mixer.music.play(-1)
...
def yellow_handle_movement(keys_pressed, yellow):
    if keys_pressed[pygame.K_LEFT] and yellow.x - VEL > BORDER.x + BORDER.width: # LEFT
        yellow.x -= VEL 
    if keys_pressed[pygame.K_RIGHT] and yellow.x + VEL + yellow.width < WIDTH + 15: # RIGHT
        yellow.x += VEL
...
def red_handle_movement(keys_pressed, red):
    if keys_pressed[pygame.K_a] and red.x - VEL > 0: # LEFT
        red.x -= VEL 
    if keys_pressed[pygame.K_d] and red.x + VEL + red.width < BORDER.x + 15: # RIGHT
        red.x += VEL
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_LCTRL and len (red_bullets) < MAX_BULLETS:
                    ...
                    pygame.mixer.music('gun_sound_effect.mp3')
            ...
            if event.type == YELLOW_HIT:
                yellow_health -= 1
                pygame.mixer.music('gun_sound_impact.mp3')
                pygame.mixer.music.play()
